...
function assertFileSystemAvailable() {
  if (isEdgeRuntime()) {
    throw new Error(
      "File system operations are not supported in edge runtimes. Please use InputFile.fromBuffer instead."
    );
  }
}
async function getFs() {
  ...
  try {
    ...
    throw new Error(
      "File system operations are not available in this runtime. Please use InputFile.fromBuffer instead."
    );
  }
}
...
var InputFile = class _InputFile {
  ...
  static fromBuffer(parts, name) {
    if (parts && !ArrayBuffer.isView(parts) && typeof parts.arrayBuffer === "function") {
      return new _InputFile(
        { type: "blob", data: parts },
        name
      );
    }
    if (typeof parts === "string") {
      return new _InputFile(
        { type: "buffer", data: new TextEncoder().encode(parts) },
        name
      );
    }
    if (parts instanceof ArrayBuffer) {
      return new _InputFile(
        { type: "buffer", data: new Uint8Array(parts) },
        name
      );
    }
  ...
  static fromPath(path, name) {
    assertFileSystemAvailable();
    return new _InputFile({ type: "path", path }, name ?? getFilename(path));
  }
  static fromPlainText(content, name) {
    return new _InputFile(
      { type: "buffer", data: new TextEncoder().encode(content) },
      name
    );
  }
  async size() {
