...
const _Client = class _Client {
  ...
  prepareRequest(method, url, headers = {}, params = {}) {
    ...
    if (this.config.selfSigned) {
      if (!this.selfSignedAgent) {
        this.selfSignedAgent = new undici.Agent({
          connect: {
      ...
      switch (headers["content-type"]) {
        ...
        case "multipart/form-data": {
          ...
          for (const [key, value] of Object.entries(params)) {
            if (value instanceof undici.File) {
  ...
  async chunkedUpload(method, url, headers = {}, originalPayload = {}, onProgress) {
    const [fileParam, file] = Object.entries(originalPayload).find(
      ([_, value]) => value instanceof undici.File || value instanceof inputFile.InputFile
    ) ?? [];
    if (!file || !fileParam) {
    ...
    if (file instanceof inputFile.InputFile) {
      ...
      const firstChunkHeaders2 = {
        ...headers,
        "content-range": `bytes 0-${firstChunkEnd2 - 1}/${size}`
      };
      const firstChunk2 = await file.slice(0, firstChunkEnd2);
      const firstPayload2 = { ...originalPayload };
      firstPayload2[fileParam] = new undici.File([firstChunk2], file.filename);
      ...
      const uploadChunk2 = async (chunk) => {
        ...
        chunkPayload[fileParam] = new undici.File([chunkBlob], file.filename);
    ...
    firstPayload[fileParam] = new undici.File([firstChunk], file.name);
    ...
    const uploadChunk = async (chunk) => {
      ...
      const chunkPayload = { ...originalPayload };
      chunkPayload[fileParam] = new undici.File([chunkBlob], file.name);
      const chunkResponse = await this.call(
        method,
        url,
        chunkHeaders,
  ...
  async redirect(method, url, headers = {}, params = {}) {
    ...
    const response = await undici.fetch(uri, {
      ...options,
      redirect: "manual"
    });
  ...
  async call(method, url, headers = {}, params = {}, responseType = "json") {
    ...
    let data = null;
    const response = await undici.fetch(uri, options);
    const warnings = response.headers.get("x-appwrite-warning");
    if (warnings) {
      warnings.split(";").forEach(
